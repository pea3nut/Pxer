...
class PxerAnalytics{
    ...
    enabled: boolean
    constructor(){
        this.uid = <string> window.localStorage.getItem("PXER_UID");
        if (!this.uid) {
            this.uid = Math.random().toString(36).replace(/[^a-z]+/g, '') 
                      +Math.random().toString(36).replace(/[^a-z]+/g, '');
            window.localStorage.setItem("PXER_UID", this.uid);
        }
        this.enabled = !window.localStorage.getItem("PXER_OPT_OUT_ANALYTICS");
    };
    /**
     * 
    ...
    postData(event: string, data: any) : void{
        ...
        data.pxer_version = window.PXER_VERION;
